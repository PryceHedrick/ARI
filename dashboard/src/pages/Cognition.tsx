...
// =============================================================================
// LEARNING ANALYTICS — Metacognitive Feedback (Retention + Focus)
// =============================================================================
  ...
  return (
    <div className="bg-[var(--bg-card)] backdrop-blur-md rounded-xl border border-[var(--border-muted)] p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-base font-semibold text-[var(--text-primary)]">Learning Analytics</h3>
        <span className="text-xs text-[var(--text-muted)]">
          {analytics.retentionMetrics.dueNow} due
        </span>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-[var(--text-muted)]">Retention rate</span>
            <span className="text-xs font-medium text-[var(--text-secondary)]">{retentionPct.toFixed(0)}%</span>
          </div>
          <div className="h-2 bg-[var(--bg-interactive)] rounded-full overflow-hidden">
            <div className="h-full bg-[var(--ari-success)]" style={{ width: `${retentionPct}%` }} />
          </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-[var(--text-muted)]">Focus ratio</span>
            <span className="text-xs font-medium text-[var(--text-secondary)]">{focusPct.toFixed(0)}%</span>
          </div>
          <div className="h-2 bg-[var(--bg-interactive)] rounded-full overflow-hidden">
            <div className="h-full bg-[var(--ari-purple)]" style={{ width: `${focusPct}%` }} />
          </div>
          <div className="text-[10px] text-[var(--text-muted)] mt-1">
            {analytics.practiceQuality.deliberateHours.toFixed(1)}h focused • {analytics.practiceQuality.distractedHours.toFixed(1)}h distracted
          </div>
        </div>

        {analytics.insights.length > 0 && (
          <div className="pt-3 border-t border-[var(--border-subtle)]">
            <div className="text-xs text-[var(--text-muted)] mb-2">Insights</div>
            <div className="space-y-2">
              {analytics.insights.slice(0, 3).map((i, idx) => (
                <div key={idx} className="text-xs text-[var(--text-secondary)]">
                  <span className="text-[var(--ari-warning)]">•</span> {i.pattern} — {i.recommendation}
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === 'overview' && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            ...
            {learningAnalytics && <LearningAnalyticsCard analytics={learningAnalytics} />}
          </div>
